...
# Strip view: The four properties correspond to 4 strips which get one of three colors
# when 3 cards placed side by side, the combined strip is all the same color or 3 diff colors
SHOW_STRIP_CARDS = False
...
numberDict = dict(zip([1, 2, 3], ['red', 'green', 'blue']))
shapeDict = dict(
    zip(['ovals', 'diamonds', 'squiggles'], ['red', 'green', 'blue']))
shadeDict = dict(zip(['open', 'solid', 'stripes'], ['red', 'green', 'blue']))

def makeCap20Deal(deck, coorOnly=False):
    cap2 = [[2,1,1,2],[2,2,1,2],[2,3,3,2],[3,3,3,3],[3,3,3,1],[3,1,1,3],[3,2,1,3],[1,3,3,2],[3,2,3,2],[3,1,3,2],[3,1,1,2],[3,2,1,2],[1,3,2,3],[2,2,2,3],[2,1,2,3],[2,1,1,3],[2,2,1,3],[2,3,2,2],[3,3,2,3],[2,3,2,1]]
    color = ['red', 'green', 'blue']
    shape = ['ovals', 'diamonds', 'squiggles']
    shade = ['open', 'solid', 'stripes']
    deal = []
    for c in cap2:
        if coorOnly:
            card = c
        else:
            card = [color[c[0]-1], c[1], shape[c[2]-1], shade[c[3]-1] ]
        deal.append(card)
        deck.remove(card)
    return deal
...
def dimSet(set):
    # dim is the number of properties that vary
    # 1 means only one property changes, so set is in 1 dim subspace etc
    dim = 4
    for pr in range(4):
        if len({set[0][pr], set[1][pr], set[2][pr]}) == 1:
            dim -= 1
    return dim
...
def findBestSet(deal, deck):
    ...
    if len(sets) == 0:
        ...
        dbPrint(" best {} goodness {} dim {}".format(sbest, sbestgoodness,
                                                     dimSet(sbest)))
        return sbest
...
outlinePath = ui.Path.rect(0, 0, width, height)
messagePath = ui.Path.rounded_rect(0, 0, width, height, 10)
...
rectPath = ui.Path.rect(0, 0, width, height / 5)
...
class Card(Node):
    def __init__(
        ...
        if SHOW_STRIP_CARDS:
            colorStrip = ShapeNode(path=rectPath, *args, **kwargs)
            colorStrip.stroke_color = color
            colorStrip.fill_color = color
            colorStrip.position = [0, 3 * height / 8]

            numberStrip = ShapeNode(path=rectPath, *args, **kwargs)
            numberStrip.stroke_color = numberDict[number]
            numberStrip.fill_color = numberDict[number]
            numberStrip.position = [0, height / 8]

            shapeStrip = ShapeNode(path=rectPath, *args, **kwargs)
            shapeStrip.stroke_color = shapeDict[shape]
            shapeStrip.fill_color = shapeDict[shape]
            shapeStrip.position = [0, -height / 8]

            shadingStrip = ShapeNode(path=rectPath, *args, **kwargs)
            shadingStrip.stroke_color = shadeDict[shade]
            shadingStrip.fill_color = shadeDict[shade]
            shadingStrip.position = [0, -3 * height / 8]

            outline = ShapeNode(path=outlinePath, *args, **kwargs)
            outline.fill_color = 'white'
            self.add_child(outline)
            self.add_child(shadingStrip)
            self.add_child(shapeStrip)
            self.add_child(numberStrip)
            self.add_child(colorStrip)
...
class MyScene(Scene):
    def setup(self):
        ...
        self.rect = ShapeNode(
            messagePath, color='#ff4300', parent=self.messageBoard)
        ...
        self.deckInfo = Node(position=(10, 700), parent=self)
        self.deckLabel = LabelNode('', parent=self.deckInfo)
        self.deckLabel.anchor_point = (0, 5)

        self.dealInfo = Node(position=(10, 675), parent=self)
    ...
    def start(self):
        ...
        self.deal = make21Deal(self.deck)
        #self.deal = makeCap20Deal(self.deck)
        # print(self.deal)
    ...
    def activateAutoPlay(self):
        ...
        if (len(self.deck) == 0) & (len(findSets(self.deal)) == 0):
            # deck empty end game but no sets left in deal
    ...
    def checkSetsDisplayed(self):
        self.numDim = 4 * [0]
        for set in self.setsDisplayed:
            self.numDim[dimSet([[
                card.color, card.number, card.shape, card.shade
            ] for card in set]) - 1] += 1
    ...
    def updateScore(self):
        ...
        self.score.text = "Set Calls: {} Correct, {} Incorrect, {} Aborted, Deals: {} Good, {} Premature, Auto: {} Sets, {} Deals, Time {}\nTotal Sets {} with dimensions {}".format(
            self.numCorrectSets, self.numBadSets, self.numAbortSetCall,
            self.numCorrectDeals, self.numBadDeals, self.numAutoSets,
            self.numAutoDeals, tstr, self.numCorrectSets + self.numAutoSets,
            self.numDim)
