...
# scale for other screen sizes
SCALESCREEN = 1
design_size=Size(1024,768)
sz=get_screen_size()
size_ratio=min([x/y for x,y in zip(tuple(sz),tuple(design_size))])
# rather than calc simply set noe if < .5 get no display on iphone 12 mini

iphone = True
if iphone:
    size_ratio = .5 # 1 for ipad
    topDeal =560 # 650 for ipad
    iphadj=585 # ypos for info lines 700 for ipad
    dispFoundAdj = -100
    dispFoundAdj2 = 50
...
class Card(Node):
    def __init__(
        ...
        if x == None:
            ...
            y *= height + 5
            y += height / 2
            y = topDeal - y # topDeal set for different devices
            self.posInd = posInd
        ...
        shading.x_scale = SCALESCREEN
        shading.y_scale = SCALESCREEN
        shading.stroke_color = color
        outline = ShapeNode(path=outlinePath, *args, **kwargs)
        outline.x_scale = SCALESCREEN
        outline.y_scale = SCALESCREEN
        ...
        self.cardShape.x_scale = SCALESCREEN
        self.cardShape.y_scale = SCALESCREEN
...
class MyScene(Scene):
    def setup(self):
        ...
        self.buttonAllAuto = ShapeNode(
            ui.Path.rounded_rect(0, 0, 60, 60, 12),
            position=(buttonXpos['allauto'], 30),
            fill_color='white',
            parent=self)

        self.buttonAuto = ShapeNode(
            ui.Path.rounded_rect(0, 0, 60, 60, 12),
            position=(buttonXpos['auto'], 30),
            fill_color='orange',
            parent=self)

        self.buttonPile = ShapeNode(
            ui.Path.rounded_rect(0, 0, 60, 60, 12),
            position=(buttonXpos['pile'], 30),
            fill_color='purple',
            parent=self)

        self.buttonSet = ShapeNode(
            ui.Path.rounded_rect(0, 0, 60, 60, 12),
            position=(buttonXpos['set'], 30),
            fill_color='red',
            parent=self)
            ...
            ctmp.run_action(
                Action.sequence(
                    Action.group(
                        ...
                        Action.move_to(self.xDisp-100, self.yDisp-
